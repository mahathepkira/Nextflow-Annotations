...
workflow Annotations {
   ...
   main:
     if (__params.vcf_compare != "") {
         ...
         (vcf_ann2,genes,html,csv) = ANN_snpEff(unique,extraOpts)
         vcf_final = Combine_VCF(vcf_ann1, vcf_ann2)
     } else {
         (vcf_final,genes,html,csv) = ANN_snpEff(vcfgz,extraOpts)
     }
...
workflow Annotations_custom {
   ...
   main:
     ...
     if (__params.vcf_compare != "") {
         (overlap, unique) = Compare_vcf(vcfgz)
         vcf_ann1 = Call_ANN(overlap)
         (vcf_ann2,genes,html,csv) = ANN_snpEff(unique,extra_opts)
         vcf_final = Combine_VCF(vcf_ann1, vcf_ann2)
     } else {
         (vcf_final,genes,html,csv) = ANN_snpEff(vcfgz,extraOpts)
     }
